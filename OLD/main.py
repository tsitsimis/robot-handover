...
while True:
    ...
    if found:
        ...
        if cv2.contourArea(contour) > 4000:
            turn_yaw, turn_pitch, head_yaw, head_pitch = head_controller(motion_proxy, cx, cy, tol=tol)
        else:
            turn_yaw = False
            turn_pitch = False
            head_yaw = 0
        ...
        if turn_yaw or turn_pitch:
            ...
            set_tf(motion_proxy, HEAD, flat_tf(T_h1))               # move head
        else:
            ...
            set_tf(motion_proxy, R_ARM, flat_tf(T_e1))              # move arm: AXIS: XYZ ??? / R1 = R0
